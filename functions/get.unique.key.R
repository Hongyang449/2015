...
get.unique.key <- function(cij, cmap=NULL, scut=4, cutoff.cij=0, extra.filter=NULL, membership=NULL, 
  intra=TRUE, inter=TRUE, aaseq=NULL, count=TRUE) {
  ...
  if(count) {
    ...
    sum <- rbind(rowSums(abs(cij1)), rowSums(abs(cij2)))
    colnames(sum) <- aaseq
    rownames(sum) <- names(cij)
    return(list(cij1=cij1, cij2=cij2, sum=sum))
  }
}
